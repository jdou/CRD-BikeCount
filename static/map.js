...
var yearList = [2011, 2012, 2013, 2014, 2015, 2016, 2017,2018]
var monthList = [5, 10]
...
function barChart(indata) {
    if (indata.length > 0) {
        var x = d3.scaleBand()
            .domain(["07:00 ", "08:00 ", "15:00 ", "16:00 ", "17:00 "])
            .range([0, width])
            .padding(.1);
        //console.log(indata)
        ...
        g.selectAll(".bar")
            ...
            .attr("class", function(d) {
                if (parseInt(hourOnlyParse(d.CountDate)) > 8) {
                    return "bar pm";
                } else {
                    return "bar am";
                }
            ...
            .attr("x", function(d) {
                return x(hourParse(d.CountDate));
            })
            .attr("y", function(d) {
                return y(d.HourTotal);
            })
            .attr("width", x.bandwidth())
            .attr("height", function(d) {
                return height - y(d.HourTotal);
            });
        return svg
    } else {
        var svg = d3.select("body")
...
map.on('click', function(e) {
    ...
    d3.json("/data/v1.0/" + feature.properties['countID'], function(
        error, data) {
        hourData = data
        ...
        selectedDayData = hourData.filter(function(d) {
            return d.countID == feature.properties[
                    'countID'] & yearParse(d.CountDate) ==
                CurrentYear & monthParse(d.CountDate) ==
                CurrentMonth;
        })
        y.domain([0, d3.max(selectedDayData, function(d) {
                return d.HourTotal;
            })])
            .nice();
        var svg = barChart(selectedDayData.filter(function(d) {
            return d.heading == "NB" || d.heading ==
                "EB"
        }))
        var svg2 = barChart(selectedDayData.filter(function(d) {
            return d.heading == "SB" || d.heading ==
                "WB"
        }))
        //Combine Both svg nodes under one parent element
        var popUpDiv = document.createElement("div")
        popUpDiv.innerHTML = '<div><h4>' + '#' + feature.properties[
                'countID'] + ' ' + feature.properties[
                'onStreet'] + " " + feature.properties[
                'location'] + " of " + feature.properties[
                'xStreet'] + "</br>Peak Hour:" + feature.properties[
                'peakHour'] + '</h4>' + '<a href="/chart/' +
...
document.getElementById('buttons-year')
